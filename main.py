...
WEBHOOK_SECRET = "supersecrethook123"  # 🔒 Change this to a strong unique string
WEBHOOK_URL = f"https://your-render-url.onrender.com{WEBHOOK_PATH}"  # replace this with your deployed Render domain
...
async def add_balance_cmd(m: Message):
    ...
    parts = m.text.split()
    if len(parts) != 3:
        return await m.answer("Usage: /addbalance <user_id> <amount>", parse_mode=None)

    try:
        uid = int(parts[1])
        amt = float(parts[2])
    except ValueError:
        return await m.answer("❌ Invalid format.", parse_mode=None)

    user = cur.execute("SELECT balance FROM users WHERE user_id = ?", (uid,)).fetchone()
    if not user:
        return await m.answer("❌ User not found.", parse_mode=None)

    cur.execute("UPDATE users SET balance = balance + ? WHERE user_id = ?", (amt, uid))
    conn.commit()

    await m.answer(f"✅ ₹{amt:.2f} added to user {uid}", parse_mode=None)
    await bot.send_message(uid, f"✅ ₹{amt:.2f} has been added to your wallet by the admin.", parse_mode=None)
